...
export async function update(action, model, data) {
  ...
  switch(action) {
    ...
    case "LOGIN":
      ...
      new_model.todos = auth.loadTodos(new_model.currentUser);
      } else {
  ...
  if (new_model.currentUser) {
    auth.saveTodos(new_model.currentUser, new_model.todos);
  }
  return new_model;
}
...
export function render_item(item, signal) {
  return li([
    ...
    div(["class=view"], [
      ...
      span(["class=deadline"], [
          text(item.deadline ? "📅 " + item.deadline : "") ]),
      button([
        "class=destroy",
