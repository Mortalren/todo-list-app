...
function mount (model, update, view, root_element_id, subscriptions) {
  ...
  function signal(action, data, model) { // signal function takes action
    return async function callback() { // and returns callback
      model = JSON.parse(localStorage.getItem(store_name)) //|| model;
      const updatedModel = await update(action, model, data); // update model for action
...
function select(attrlist, childnodes) {
    return create_element("select", attrlist, childnodes);
}

function option(attrlist, childnodes) {
    return create_element("option", attrlist, childnodes);
}
